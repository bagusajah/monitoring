...
remitboothsend_sukses = Gauge('remit_boothsend_sukses', 'Total booth send sukses')
...
def dailybooth_send_success():
    nama_fungsi = 'boothsend_success'

    my_connection(nama_fungsi)
    my_cur = my_conn.cursor()
    my_cur.execute("SELECT `SEND SUCCESS` FROM TRX_BOOTH_SEND_SUCCESS;")

    rows = my_cur.fetchone()
    for data in rows:
        hasil = data[0]

    remitboothsend_sukses.set(hasil)
...
if __name__ == '__main__':
    ...
    while True:
         try:
             ...
             daily_mon_remitbooth_thread = Thread(target=dailybooth_send_success)
             daily_mon_remit_thread.start()
             daily_mon_remitbooth_thread.start()
             time.sleep(10)
